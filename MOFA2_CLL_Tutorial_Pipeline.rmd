...
- ✅ Avoids Python/mofapy2 compatibility issues that affect model training
- ✅ Guarantees reproducibility with published results
- ✅ Allows immediate focus on analysis and interpretation
- ✅ Demonstrates the full MOFA2 analysis workflow
...
# ⚠️ Note: This package is large, increase timeout if needed
options(timeout = 600)
...
# Display dimensions of each modality (features × samples)
cat("=== Data Modality Dimensions (features × samples) ===\n")
for (view in names(CLL_data)) {
  dims <- dim(CLL_data[[view]])
  cat(sprintf("%s: %d features × %d samples\n", view, dims[1], dims[2]))
}
```
...
CLL_metadata <- tryCatch({
  ...
  cat("⚠️ Could not load metadata from FTP. Creating minimal metadata...\n")
...
> 💡 **Learning Point**: In your own analyses, you would train your model using `run_mofa()` after setting up a compatible Python environment.
...
cat("• data: Input data matrices\n")
cat("• expectations: Factor values (Z) and Feature weights (W)\n")
cat("• samples_metadata: Clinical covariates\n")
cat("• dimensions: Number of views, groups, samples, factors\n")
```
...
# 
# Z (Factors): samples × factors matrix
...
#
# W (Weights): features × factors matrix (one per view)
...
cat("Dimensions:", Z_dims[1], "samples ×", Z_dims[2], "factors\n")
...
for (view in names(MOFAobject@expectations$W)) {
  ...
  cat(sprintf("• %s: %d features × %d factors\n", view, W_dims[1], W_dims[2]))
}
...
cat("Interpretation:\n")
cat("• Dark colors = high variance explained (important factor for that view)\n")
cat("• Light colors = low variance explained\n")
cat("• Factors active across multiple views = shared biological signals\n\n")
...
1. **Factor 1** captures variance across **ALL** modalities → likely represents a major disease driver
2. **Factor 2** is highly specific to the **Drug response** view → drug-specific effect
3. **Factor 3** captures variation across Drugs, mRNA, and Mutations (not Methylation) → another biological signal
...
> 💡 **Learning Point**: With only 15 factors, MOFA explains ~54% of Drug response variation and ~42% of mRNA variation. This is excellent for a linear model!
...
# - Data is centered → values spread around zero
# ============================================================================
...
> 🎯 **Key Finding**: IGHV mutation has the **largest weight** for Factor 1! This confirms Factor 1 captures IGHV-related biology.
...
> 🎯 **Key Finding**: Trisomy 12 is the **major driver** of Factor 3! This is the second most important clinical marker in CLL.
...
> 🏥 **Clinical Implication**: These molecular subgroups can guide **precision medicine** treatment decisions!
...
- ✅ **Precision medicine**: Stratify patients for targeted therapy
- ✅ **Biomarker discovery**: Identify genes predictive of drug response
- ✅ **Survival prediction**: Factors correlate with time-to-treatment
- ✅ **Drug repurposing**: Find drugs effective in specific molecular subgroups

## Next Steps for Your Own Analysis

1. **Prepare your data**: Format as list of matrices (features × samples)
